...
namespace StreetSignal\Modules\V5\Http\Resources;
...
class PostValueCollection extends ResourceCollection
{
     ...
     * @var string
     */
    public $collects = 'StreetSignal\Modules\V5\Http\Resources\PostValueResource';
        ...
        $tasks = $tasks->map(function ($task, $key) use ($fields_by_task) {
            ...
            $task['fields'] = $fields->map(function ($field, $key) use ($values) {
                ...
                if (!empty($field['value'])) {
                    if (get_class($field['value']) === 'StreetSignal\Modules\V5\Http\Resources\PostValueResource') {
