    ...
    protected function savePostValues(Post $post, array $post_content, int $post_id)
    {
        ...
        foreach ($post_content as $stage) {
            ...
            foreach ($stage['fields'] as $field) {
                ...
                $class_name = "StreetSignal\Modules\V5\Models\PostValues\Post" . ucfirst($type);
                if (!class_exists($class_name) &&
                    in_array(
                        $class_name,
                        [
                            'StreetSignal\Modules\V5\Models\PostValues\PostTitle',
                            'StreetSignal\Modules\V5\Models\PostValues\PostDescription'
                        ]
                    )
                ) {
                    continue;
