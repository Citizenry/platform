...
class CoreConfig extends ContainerConfig
{
    ...
    public function define(Container $di): void
    {
        ...
        $di->set('factory.validator', $di->lazyNew(\StreetSignal\Modules\V3\Factory\ValidatorFactory::class));

        // Implemented validators will be mapped to resources and actions.
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map'] = [];

        // Authorizers are used to check if the accessing user has permission to use an action.
        $di->set('factory.authorizer', $di->lazyNew(\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class));

        // Authorizers are shared, so mapping is done with service names.
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map'] = [
        ...
        $di->set('factory.repository', $di->lazyNew(\StreetSignal\Modules\V3\Factory\RepositoryFactory::class));

        // Repositories are shared, so mapping is done with service names.
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map'] = [
        ...
        $di->set('factory.formatter', $di->lazyNew(\StreetSignal\Modules\V3\Factory\FormatterFactory::class));

        // Implemented collection formatter will register as the factory.
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['factory'] = null;
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['collections'] = [
            'search' => true,
            'update_collection' => true
        ];

        // Data transfer objects are used to carry complex search filters between collaborators.
        $di->set('factory.data', $di->lazyNew(\StreetSignal\Modules\V3\Factory\DataFactory::class));

        // Usecases that perform searches are the most typical usage of data objects.
        $di->params[\StreetSignal\Modules\V3\Factory\DataFactory::class]['actions'] = [
            'search' => $di->lazyNew(\StreetSignal\Core\Tool\SearchData::class),
            'stats'  => $di->lazyNew(\StreetSignal\Core\Tool\SearchData::class),
            'export'  => $di->lazyNew(\StreetSignal\Core\Tool\SearchData::class),
        ];

        // Use cases are used to join multiple collaborators together for a single interaction.
        $di->set('factory.usecase', $di->lazyNew(\StreetSignal\Modules\V3\Factory\UsecaseFactory::class));
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['actions'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map'] = [];

        // Config does not allow ordering or sorting, because of its simple key/value nature.
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['config'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['form_attributes'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['form_roles'] = [
            'update_collection'  => $di->lazyNew(Usecase\Form\UpdateFormRole::class),
            'search'  => $di->lazyNew(Usecase\Form\SearchFormRole::class),
        ];

        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['form_contacts'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['form_stats'] = [
            'search'  => $di->lazyNew(Usecase\Form\SearchFormStats::class),
        ];

        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['form_stages'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['media'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['csv'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['messages'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['posts'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['notifications'] = [
            'create'  => $di->lazyNew(Usecase\Notification\CreateNotification::class)
        ];

        // Add custom create usecase for webhooks
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['webhooks'] = [
            'create'  => $di->lazyNew(Usecase\Webhook\CreateWebhook::class)
        ];

        // Add custom create usecase for export jobs
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['export_jobs'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['contacts'] = [
            'create'  => $di->lazyNew(Usecase\Contact\CreateContact::class)
        ];

        // Add custom create usecase for terms of service
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['tos'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['sets_posts'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['posts_lock'] = [
        ...
        // Add custom usecases for sets_posts
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['savedsearches'] = [
            'create' => $di->lazyNew(Usecase\Set\CreateSet::class),
        ];

        // Add custom usecases for sets_posts
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['sets'] = [
            'create' => $di->lazyNew(Usecase\Set\CreateSet::class),
        ];

        // Add usecase for posts_export
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['posts_export'] = [
        ...
        // repositories for StreetSignal\Contracts\Repository\Usecase\Post\Export usecase
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['users'] = [
        ...
        // Traits
        $di->setters[\StreetSignal\Core\Concerns\UserContext::class]['setSession'] = $di->lazyGet('session');
        ...
        $di->setters[\StreetSignal\Core\Concerns\Event::class]['setEmitter'] = $di->lazyNew(\League\Event\Emitter::class);

        // Set post permissions instance
        $di->setters[\StreetSignal\Core\Tool\Permissions\InteractsWithPostPermissions::class]['setPostPermissions']
            = $di->lazyNew(\StreetSignal\Core\Tool\Permissions\PostPermissions::class);

        // Set form permissions instance
        $di->setters[\StreetSignal\Core\Tool\Permissions\InteractsWithFormPermissions::class]['setFormPermissions']
            = $di->lazyNew(\StreetSignal\Core\Tool\Permissions\FormPermissions::class);

        // Set ACL for ACL Trait
        $di->setters[\StreetSignal\Core\Concerns\Acl::class]['setAcl'] = $di->lazyGet('tool.acl');

        // Tools
        $di->set('tool.signer', $di->lazyNew(\StreetSignal\Core\Tool\Signer::class));
        $di->set('tool.verifier', $di->lazyNew(\StreetSignal\Core\Tool\Verifier::class, [
            'apiKeyRepo' => $di->lazyGet('repository.apikey')
        ]));
        $di->set('tool.uploader', $di->lazyNew(\StreetSignal\Core\Tool\Uploader::class));
        $di->params[\StreetSignal\Core\Tool\Uploader::class] = [
            'fs' => $di->lazyGet('tool.filesystem'),
            'multisite' => $di->lazyGet('multisite'),
        ];

        $di->set('tool.acl', $di->lazyNew(\StreetSignal\Core\Tool\Acl::class));
        $di->setters[\StreetSignal\Core\Tool\Acl::class]['setRoleRepo'] = $di->lazyGet('repository.role');

        $di->set('tool.hasher.password', $di->lazyNew(\StreetSignal\Core\Tool\Hasher\Password::class));
        $di->set('tool.authenticator.password', $di->lazyNew(\StreetSignal\Core\Tool\Authenticator\Password::class));

        $di->set('filereader.csv', $di->lazyNew(\StreetSignal\Core\Tool\FileReader\CSV::class));
        $di->setters[\StreetSignal\Core\Tool\FileReader\CSV::class]['setReaderFactory'] =
            $di->lazyGet('csv.reader_factory');

        $di->set('csv.reader_factory', $di->lazyNew(\StreetSignal\Core\Tool\FileReader\CSVReaderFactory::class));
        ...
        $di->setters[\BehEh\Flaps\Flap::class]['setViolationHandler'] =
            $di->lazyNew(\StreetSignal\Core\Tool\ThrottlingViolationHandler::class);
        ...
        $di->set('ratelimiter.register', $di->lazyNew(
            \StreetSignal\Core\Tool\RateLimiter::class,
            [
