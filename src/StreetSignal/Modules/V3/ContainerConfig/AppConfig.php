...
namespace StreetSignal\Modules\V3\ContainerConfig;

use StreetSignal\Core;
use StreetSignal\Modules\V3;
...
class AppConfig extends ContainerConfig
{
    ...
    public function define(Container $di): void
    {
        ...
        // Validator mapping
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['apikeys'] = [
            'create' => $di->lazyNew(V3\Validator\ApiKey\Create::class),
            'update' => $di->lazyNew(V3\Validator\ApiKey\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['config'] = [
            'update' => $di->lazyNew(V3\Validator\Config\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['forms'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['layers'] = [
            'create' => $di->lazyNew(V3\Validator\Layer\Create::class),
            'update' => $di->lazyNew(V3\Validator\Layer\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['media'] = [
            'create' => $di->lazyNew(V3\Validator\Media\Create::class),
            'delete' => $di->lazyNew(V3\Validator\Media\Delete::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['posts'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['export_jobs'] = [
            'create' => $di->lazyNew(V3\Validator\ExportJob\Create::class),
            'update' => $di->lazyNew(V3\Validator\ExportJob\Update::class),
        ];

        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['posts_lock'] = [
            'create' => $di->lazyNew(V3\Validator\Post\Create::class),
            'update' => $di->lazyNew(V3\Validator\Post\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['tags'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['users'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['user_settings'] = [
            'create' => $di->lazyNew(V3\Validator\User\Setting\Create::class),
            'update' => $di->lazyNew(V3\Validator\User\Setting\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['messages'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['roles'] = [
            'create' => $di->lazyNew(V3\Validator\Role\Create::class),
            'update' => $di->lazyNew(V3\Validator\Role\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['permissions'] = [
            'create' => $di->lazyNew(V3\Validator\Permission\Create::class),
            'update' => $di->lazyNew(V3\Validator\Permission\Update::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['tos'] = [
            'create' => $di->lazyNew(V3\Validator\Tos\Create::class),
        ];

        // Formatter mapping
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['factory']
            = $di->newFactory(V3\Formatter\Collection::class);


        $di->set('tool.jsontranscode', $di->lazyNew(\StreetSignal\Core\Tool\JsonTranscode::class));
        ...
        $di->params[V3\Repository\PostRepository::class] = [
            ...
            'bounding_box_factory' => $di->newFactory(\StreetSignal\Core\Tool\BoundingBox::class)
        ];
        ...
        $di->set('authorizer.hxl', $di->lazyNew('StreetSignal\Core\Tool\Authorizer\HXLAuthorizer'));

        $di->params[StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl'] =
        ...
        $di->set(
            ...
            $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\HXLMetadataAuthorizer::class)
        );

        $di->setters[V3\Formatter\HXL\HXLMetadata::class]['setAuth']
            = $di->lazyGet("authorizer.hxl.meta_data");
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['hxl_meta_data'] = [
            'create' => $di->newFactory(Core\Usecase\HXL\Metadata\Create::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl_meta_data'] =
            $di->lazyGet('authorizer.hxl.meta_data');
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_meta_data'] =
            $di->lazyGet('repository.hxl_meta_data');
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['hxl_meta_data'] =
            $di->lazyNew(V3\Formatter\HXL\HXLMetadata::class);
        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['hxl_meta_data'] = [
        ...
        $di->set(
            ...
            $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\HXLAuthorizer::class)
        );
        $di->setters[V3\Formatter\HXL\HXLFormAttributeHXLAttributeTagFormatter::class]['setAuth']
            = $di->lazyGet("authorizer.hxl");
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['form_attribute_hxl_attribute_tag'] =
            $di->lazyGet('authorizer.hxl.form_attribute_hxl_attribute_tag');
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['form_attribute_hxl_attribute_tag'] =
            $di->lazyGet('repository.form_attribute_hxl_attribute_tag');
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['form_attribute_hxl_attribute_tag'] =
            $di->lazyNew(V3\Formatter\HXL\HXLFormAttributeHXLAttributeTagFormatter::class);
        $di->setters[V3\Formatter\HXL\HXLFormAttributeHXLAttributeTagFormatter::class]['setAuth'] =
            $di->lazyGet("authorizer.hxl");

        $di->params[\StreetSignal\Modules\V3\Factory\ValidatorFactory::class]['map']['form_attribute_hxl_attribute_tag'] = [
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_attributess'] =
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl_licenses'] =
            $di->lazyGet('authorizer.hxl');
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_licenses'] =
            $di->lazyGet('repository.hxl_license');
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['hxl_licenses'] =
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl_tags'] =
            $di->lazyGet('authorizer.hxl');
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_tags'] =
            $di->lazyGet('repository.hxl_tag');
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['hxl_tags'] =
            $di->lazyNew(V3\Formatter\HXL\HXLTag::class);
        $di->setters[V3\Formatter\HXL\HXLTag::class]['setAuth'] =
            $di->lazyGet("authorizer.hxl");

        // repositories for StreetSignal\Core\Usecase\HXL\SendHXLUsecase
        ...
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_send'] =
            $di->lazyGet('repository.hxl_tag'); //FIXME
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['hxl_send'] = [
            'send' => $di->lazyNew(Core\Usecase\HXL\SendHXLUsecase::class),
        ];

        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['hxl_send'] =
            $di->lazyNew(V3\Formatter\HXL\HXLSend::class);
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl_send'] =
            $di->lazyGet('authorizer.hxl');
        // add organisations
        $di->params[\StreetSignal\Modules\V3\Factory\UsecaseFactory::class]['map']['hxl_organisations'] = [
            'search' => $di->newFactory(Core\Usecase\HXL\Organisations\GetByUser::class),
        ];
        $di->params[\StreetSignal\Modules\V3\Factory\FormatterFactory::class]['map']['hxl_organisations'] =
            $di->lazyNew(V3\Formatter\HXL\HXLOrganisations::class);
        $di->params[\StreetSignal\Modules\V3\Factory\AuthorizerFactory::class]['map']['hxl_organisations'] =
            $di->lazyGet('authorizer.hxl');
        ...
        $di->setters[Core\Usecase\HXL\Organisations\GetByUser::class]['setRepository'] =
            null;
        $di->params[\StreetSignal\Modules\V3\Factory\RepositoryFactory::class]['map']['hxl_organisations'] =
            $di->lazyGet('repository.hxl_tag'); //FIXME

        // Authorizer
        $di->set('authorizer.config', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ConfigAuthorizer::class));
        $di->set('authorizer.console', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ConsoleAuthorizer::class));
        $di->set(
            'authorizer.dataprovider',
            $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\DataProviderAuthorizer::class)
        );
        $di->set('authorizer.form', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\FormAuthorizer::class, [
            'form_repo' => $di->lazyGet('repository.form'),
        ]));
        $di->params[\StreetSignal\Modules\V5\Policies\SurveyPolicy::class] = [
            'form_repo' => $di->lazyGet('repository.form'),
        ];
        $di->set(
            'authorizer.form_attribute',
            $di->lazyNew(
                \StreetSignal\Core\Tool\Authorizer\FormAttributeAuthorizer::class,
                [
                'stage_repo' => $di->lazyGet('repository.form_stage'),
        ...
        $di->set('authorizer.form_role', $di->lazyNew(
            \StreetSignal\Core\Tool\Authorizer\FormRoleAuthorizer::class,
            [
                'form_repo' => $di->lazyGet('repository.form'),
        ...
        $di->set(
            ...
            $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\FormStageAuthorizer::class, [
                'form_repo' => $di->lazyGet('repository.form'),
                'form_auth' => $di->lazyGet('authorizer.form'),
            ])
        );
        $di->set('authorizer.form_contact', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\FormContactAuthorizer::class, [
            'form_repo' => $di->lazyGet('repository.form'),
            'form_auth' => $di->lazyGet('authorizer.form'),
        ]));
        $di->set('authorizer.form_stats', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\FormStatsAuthorizer::class));
        $di->set('authorizer.user', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\UserAuthorizer::class));
        $di->set('authorizer.user_setting', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\UserSettingAuthorizer::class));
        $di->set('authorizer.layer', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\LayerAuthorizer::class));
        $di->set('authorizer.media', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\MediaAuthorizer::class));
        $di->set('authorizer.message', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\MessageAuthorizer::class));
        $di->set('authorizer.tag', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\TagAuthorizer::class));
        $di->set('authorizer.savedsearch', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\SetAuthorizer::class));
        $di->set('authorizer.set', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\SetAuthorizer::class));
        $di->set(
            'authorizer.notification',
            $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\NotificationAuthorizer::class)
        );
        $di->set('authorizer.webhook', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\WebhookAuthorizer::class));
        $di->set('authorizer.apikey', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ApiKeyAuthorizer::class));
        $di->set('authorizer.contact', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ContactAuthorizer::class));
        $di->set('authorizer.csv', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\CSVAuthorizer::class));
        $di->set('authorizer.role', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\RoleAuthorizer::class));
        $di->set('authorizer.permission', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\PermissionAuthorizer::class));
        $di->set('authorizer.post', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\PostAuthorizer::class));
        $di->set('authorizer.post_lock', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\PostAuthorizer::class));
        $di->set('authorizer.tos', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\TosAuthorizer::class));
        $di->set('authorizer.external_auth', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ExternalAuthorizer::class));
        $di->set('authorizer.export_job', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\ExportJobAuthorizer::class));
        $di->params[\StreetSignal\Core\Tool\Authorizer\PostAuthorizer::class] = [
            'post_repo' => $di->lazyGet('repository.post'),
            'form_repo' => $di->lazyGet('repository.form'),
        ];
        $di->params[\StreetSignal\Core\Tool\Authorizer\TagAuthorizer::class] = [
            'tag_repo' => $di->lazyGet('repository.tag'),
        ];

        $di->set('authorizer.country_code', $di->lazyNew(\StreetSignal\Core\Tool\Authorizer\CountryCodeAuthorizer::class));
