...
Feature: Testing OAuth2 endpoints
    ...
    Scenario: Requesting access token with password
        ...
        """
          grant_type=password&client_id=demoapp&client_secret=demopass&username=robbie@streetsignal.com&password=testing
        """
    ...
    Scenario: Requesting access token with incorrect password fails
        ...
        And that the request "data" is:
        """
          grant_type=password&client_id=demoapp&client_secret=demopass&username=robbie@streetsignal.com&password=wrongpassword
        """
        And that the api_url is ""
        Then I request "oauth/token"
        Then the response is JSON
